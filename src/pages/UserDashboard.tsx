...
>
 ...
 profile?.admin_approval === 0 && (
  <p className="text-xs text-orange-600 mt-2">
    Reward wallet will be enabled after admin approval
  </p>
)}
...
<div className="flex items-center justify-between mb-4 px-6">
      ...
      {showOrderModal && selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 max-w-lg w-full">
            <h3 className="text-xl font-bold text-slate-900 mb-4">Order #{selectedOrder.id}</h3>
            ...
            <div className="grid grid-cols-1 gap-2 text-sm text-slate-700 mb-4">
              ...
              <div><strong>Material:</strong> {selectedOrder.material_type}</div>
              <div><strong>Weight:</strong> {selectedOrder.weight}</div>
              <div><strong>Vehicle:</strong> {selectedOrder.vehicle_type} {selectedOrder.wheel_type ? `- ${selectedOrder.wheel_type}` : ''}</div>
              <div><strong>Amount:</strong> ₹{selectedOrder.amount}</div>
              <div><strong>Advance:</strong> ₹{selectedOrder.advance}</div>
  ...
  <button
    ...
    className="flex-1 py-2 rounded-lg bg-slate-100 hover:bg-slate-200"
  >
    Close
  </button>

  <button
    onClick={() => handleShareOrderLink(selectedOrder)}
