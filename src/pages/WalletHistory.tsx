...
const rawWheel =
  ((user as any)?.wheel_type || '').toString().toLowerCase();
...
const fetchWalletHistory = async () => {
  ...
  // fixed reward amount: 500 (elite) or 1500 (premium)
  const rewardWithdrawAmount =
    membershipType === 'elite' ? MIN_REWARD_ELITE : MIN_REWARD_PREMIUM;

  // amount we will actually withdraw
  const withdrawAmount =
    walletType === 'reward' ? rewardWithdrawAmount : balance;

  // validation
  if (walletType === 'reward') {
    if (balance < rewardWithdrawAmount) {
      alert(
        `You need at least ₹${rewardWithdrawAmount} in your Reward wallet to withdraw.`
      );
      return;
    }
  } else {
  ...
  // 👇 NEW TEXT – use this to confirm you’re on the new build
   // confirm with the actual amount (500 / 1500 / diesel full)
  if (
    !confirm(
      `Withdraw ₹${withdrawAmount} from ${
        walletType === 'reward' ? 'Reward' : 'Diesel'
      } wallet?`
    )
  ) {
    return;
    ...
    const resp = await fetch(
      'https://projects.growtechnologies.in/bhadra/api/wallet_withdraw.php',
      {
        ...
        body: JSON.stringify({
          ...
          wallet: walletType,
          amount: withdrawAmount, // 500 / 1500 / diesel full
        }),
      }
    );
...
<div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 flex flex-col justify-between">
  <div>
    ...
    {/* Dynamic text based on membership */}
    {membershipType === 'elite' ? (
      <p className="text-xs text-slate-500 mt-2">
        Elite Member – You earn ₹500 per referral.
        <br />
        Minimum withdrawal: ₹{MIN_REWARD_ELITE}
      </p>
    ) : (
      <p className="text-xs text-slate-500 mt-2">
        Premium Member – You earn ₹10 per referral.
        <br />
        Minimum withdrawal: ₹{MIN_REWARD_PREMIUM}
      </p>
    )}
  </div>
