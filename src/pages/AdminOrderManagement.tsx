            ...
            <div className="space-y-3 mb-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              {selectedOrder.assigned_to && (
                <>
                  ...
                  {(() => {
                    const assignedId = String(selectedOrder.assigned_to).trim();
                    const assignedUser = users.find((u) => String(u.id).trim() === assignedId);

                    if (assignedUser && assignedUser.referral_code) {
                      return (
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-slate-600">Referral Code:</span>
                          <span className="font-mono font-medium text-blue-600 bg-blue-50 px-2 py-1 rounded">{assignedUser.referral_code}</span>
                        </div>
                      );
                    }
                    return null;
