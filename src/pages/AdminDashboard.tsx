...
const [activeTab, setActiveTab] = useState<
  'users' | 'pending' | 'orders' | 'agents' | 'withdraws' | 'wallet' >('pending');
...
const handleApproveWallet = async (userId: number | string) => {
  if (!confirm('Approve ₹2999 reward wallet?')) return;
...
const ordersArr = rawOrders
  .map(mapOrder)
  .filter((o) => {
    if (!o.created_at) return true; // safety
    const createdTime = new Date(o.created_at).getTime();
    return now - createdTime <= FOUR_HOURS;
  });
  ...
  <div className="fixed inset-0 z-50 flex">
    ...
    <div className="relative w-72 bg-white h-full shadow-xl p-4">
      ...
      {[
        ...
        { key: 'withdraws', label: `Withdraw Requests (${withdrawRequests.length})` },
